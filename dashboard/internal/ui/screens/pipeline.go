...
type PipelineModel struct {
	...
	referralApps  []model.CareerApplication
...
func (m PipelineModel) handleKey(msg tea.KeyMsg) (PipelineModel, tea.Cmd) {
	isReferral := pipelineTabs[m.activeTab].filter == filterReferrals
	...
	case "c":
		if isReferral || len(m.filtered) == 0 {
			return m, nil
		}
		m.statusPicker = true
		m.statusCursor = 0
...
func (m *PipelineModel) applyFilterAndSort() {
	...
	// Referrals tab: use referral data directly, no sorting needed
	if currentFilter == filterReferrals {
		var matched []model.CareerApplication
		for _, r := range m.referralApps {
			if matchesSearch(r, m.searchQuery) {
...
func (m PipelineModel) countForFilter(filter string) int {
	if filter == filterReferrals {
		return len(m.referralApps)
	}
	count := 0
	for _, app := range m.apps {
		norm := data.NormalizeStatus(app.Status)
...
func (m PipelineModel) renderBody() string {
	...
	for i := range m.filtered {
		...
		if isReferral {
			selected := i == m.cursor
			line := m.renderReferralLine(app, selected)
...
func (m PipelineModel) renderColumnHeader() string {
	isReferral := pipelineTabs[m.activeTab].filter == filterReferrals
	if isReferral {
		return m.renderReferralColumnHeader()
	}
...
// renderReferralColumnHeader renders the column header for the referrals tab.
func (m PipelineModel) renderReferralColumnHeader() string {
	cw := m.referralColumnWidths()
	h := lipgloss.NewStyle().Foreground(m.theme.Subtext).Bold(true)
	cell := func(label string, width int) string {
		return h.Width(width).Render(truncateRunes(label, width))
	}
...
type referralColWidths struct {
	num, date, company, contact, role, status int
}

func (m PipelineModel) referralColumnWidths() referralColWidths {
	c := referralColWidths{num: 4, date: 10, company: 16, contact: 20, status: 14}
	fixed := c.num + c.date + c.company + c.contact + c.status
	c.role = m.width - fixed - 14
	if c.role < 15 {
		c.role = 15
	}
	return c
...
// renderReferralLine renders one row of the Referrals tab: #, DATE, COMPANY,
// CONTACT, ROLE, STATUS. Mirrors renderAppLine, minus score/location/pay.
func (m PipelineModel) renderReferralLine(app model.CareerApplication, selected bool) string {
	padStyle := lipgloss.NewStyle().Padding(0, 2)
	cw := m.referralColumnWidths()
	...
	contactText := app.Contact
	if contactText == "" {
		contactText = "—"
	}
	contactStyle := lipgloss.NewStyle().Foreground(m.theme.Text).Width(cw.contact)
	...
	statusStyle := lipgloss.NewStyle().Width(cw.status).Foreground(m.theme.Subtext)
	if statusColor != "" {
	...
	segments := []string{
		...
		companyStyle.Render(truncateRunes(app.Company, cw.company)),
		contactStyle.Render(truncateRunes(contactText, cw.contact)),
		roleStyle.Render(truncateRunes(app.Role, cw.role)),
		statusStyle.Render(truncateRunes(statusText, cw.status)),
	}
