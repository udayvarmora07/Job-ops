...
type referralJSON struct {
	...
	AskedDate string `json:"askedDate"`
}

// ParseReferrals reads data/referrals.json and maps each referral request onto
// a CareerApplication so the pipeline screen's Referrals tab can render them
// with the same machinery as tracker rows. Missing/invalid file → empty slice.
func ParseReferrals(careerOpsPath string) []model.CareerApplication {
	filePath := filepath.Join(careerOpsPath, "data", "referrals.json")
	content, err := os.ReadFile(filePath)
	if err != nil {
		// Fall back to a root-level file, mirroring ParseApplications' layout tolerance.
		filePath = filepath.Join(careerOpsPath, "referrals.json")
		content, err = os.ReadFile(filePath)
		if err != nil {
			return nil
		}
	...
	var raw []referralJSON
	if err := json.Unmarshal(content, &raw); err != nil {
		return nil
	}

	apps := make([]model.CareerApplication, 0, len(raw))
	for i, r := range raw {
...
var (
	...
	reTableStatusReplacer = regexp.MustCompile(`^((?:\|[^|]*){5}\|\s*)([^|]+)(\s*\|.*)$`)
)
...
func UpdateApplicationStatus(careerOpsPath string, app model.CareerApplication, newStatus string) error {
	...
	for i, line := range lines {
		...
		// Match by report number (apps with reports)
		...
		// Fallback: match by company + role (apps without reports)
		if app.ReportNumber == "" && app.Company != "" && app.Role != "" {
			fields := parseLineFields(line)
			if len(fields) >= 4 && normalizeName(fields[2]) == normalizeName(app.Company) && normalizeName(fields[3]) == normalizeName(app.Role) {
				lines[i] = replaceStatusInLine(line, app.Status, newStatus)
				found = true
				break
			}
	...
	if !found {
		what := app.ReportNumber
		if what == "" {
			what = app.Company + " / " + app.Role
		}
		return fmt.Errorf("application not found: %s", what)
	}

	return os.WriteFile(filePath, []byte(strings.Join(lines, "\n")), 0644)
}

// parseLineFields splits a markdown table line into pipe-delimited fields.
func parseLineFields(line string) []string {
	line = strings.TrimSpace(line)
	line = strings.Trim(line, "|")
	if strings.Contains(line, "\t") {
		parts := strings.Split(line, "\t")
...
// normalizeName lowercases and trims spaces for fuzzy name comparison.
func normalizeName(name string) string {
	return strings.ToLower(strings.TrimSpace(name))
}

// replaceStatusInLine replaces the status field (6th column) in a piped markdown
// table line. Uses the pre-compiled reTableStatusReplacer regex to match the
// status column by position, avoiding false matches in other columns.
func replaceStatusInLine(line, oldStatus, newStatus string) string {
	// Pipe-delimited format: match status by column position
	if strings.Contains(line, "|") && !strings.Contains(line, "\t") {
		if m := reTableStatusReplacer.FindStringSubmatchIndex(line); m != nil {
			// m[4]:m[5] = group 2 (status field) start/end
			return line[:m[4]] + newStatus + line[m[5]:]
		}
	}
	// Tab-separated mixed format: find the 6th tab-separated field
	if strings.Contains(line, "\t") {
		parts := strings.Split(line, "\t")
		if len(parts) >= 6 {
