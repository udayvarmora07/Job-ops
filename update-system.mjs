...
/**
 * update-system.mjs — Safe auto-updater for jobops
 *
...
const CANONICAL_REPO = 'https://github.com/santifer/jobops.git';
const RAW_VERSION_URL = 'https://raw.githubusercontent.com/santifer/jobops/main/VERSION';
const RELEASES_API = 'https://api.github.com/repos/santifer/jobops/releases/latest';

// Matches a semver, with or without a leading `v` and an optional
// Release Please component prefix (e.g. `jobops-v1.9.0` → `1.9.0`).
...
async function check() {
  ...
  const [rawVersion, releaseRaw] = await Promise.all([
    ...
    curlGet(RELEASES_API, [
      ...
      '--header', 'User-Agent: jobops-update-checker',
    ]),
  ]);
...
async function apply() {
  ...
  const isReexec = process.env.JOBOPS_UPDATE_REEXEC === '1';
  ...
  try {
    ...
    const backupBranch = process.env.JOBOPS_UPDATE_BACKUP_BRANCH || updateBackupBranchName(local);
    if (!isReexec) {
      try {
    ...
    if (!isReexec) {
      try {
        ...
        execFileSync(process.execPath, ['update-system.mjs', 'apply'], {
          ...
          env: {
            ...process.env,
            JOBOPS_UPDATE_REEXEC: '1',
            JOBOPS_UPDATE_BACKUP_BRANCH: backupBranch,
          },
        });
        return;
      } catch (err) {
