...
Jobops transforma qualquer CLI de código com IA em uma central completa de busca de emprego. Em vez de acompanhar candidaturas manualmente em planilha, você tem um pipeline com IA que:
...
Construído por alguém que usou isso para avaliar 740+ vagas, gerar 100+ CVs personalizados e conquistar uma posição de Head of Applied AI. [Leia o estudo de caso completo](https://santifer.io/jobops-system).
...
Isso clona o último release em `./jobops` e instala as dependências. Depois:
...
**No primeiro uso, o jobops conduz você pela configuração — seu CV, perfil e vagas-alvo — apenas conversando. Nada para editar à mão.**
...
```
/jobops                → Mostrar todos os comandos disponíveis
/jobops {cole um JD}   → Auto-pipeline completo (avaliar + PDF + tracker)
/jobops scan           → Escanear portais por novas vagas
/jobops pdf            → Gerar CV otimizado para ATS
/jobops batch          → Avaliar múltiplas vagas em lote
/jobops tracker        → Ver status das candidaturas
/jobops apply          → Preencher formulários de candidatura com IA
/jobops pipeline       → Processar URLs pendentes
/jobops contacto       → Mensagem de outreach no LinkedIn
/jobops deep           → Pesquisa aprofundada da empresa
/jobops training       → Avaliar um curso/certificação
/jobops project        → Avaliar um projeto de portfólio
```

Ou apenas cole uma URL ou descrição de vaga diretamente -- jobops detecta automaticamente e roda o pipeline completo.
...
Sou o Santiago -- Head of Applied AI, ex-fundador (criei e vendi uma empresa que ainda opera com meu nome). Eu construí o jobops para gerenciar minha própria busca de emprego. Funcionou: usei o sistema para conquistar meu cargo atual.
...
**jobops é uma ferramenta local e open source — NÃO é um serviço hospedado.** Ao usar este software, você reconhece que:
