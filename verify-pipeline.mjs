#!/usr/bin/env node
/**
 * verify-pipeline.mjs — Health check for jobops pipeline integrity
 *
 * Checks:
 ...
 *
 * Run: node jobops/verify-pipeline.mjs
 */
...
const APPS_FILE = process.env.JOBOPS_TRACKER
...
const ADDITIONS_DIR = join(JOBOPS, 'batch/tracker-additions');
// JOBOPS_REPORTS overrides the reports dir (used by tests, mirrors JOBOPS_TRACKER).
const REPORTS_DIR = process.env.JOBOPS_REPORTS || join(JOBOPS, 'reports');
const STATES_FILE = existsSync(join(JOBOPS, 'templates/states.yml'))
  ? join(JOBOPS, 'templates/states.yml')
  : join(JOBOPS, 'states.yml');
...
for (const e of entries) {
  ...
  if (!match) continue;
  const link = match[1];
  if (!existsSync(join(TRACKER_DIR, link)) && !existsSync(join(JOBOPS, link))) {
