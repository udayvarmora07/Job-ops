...
/**
 * test-all.mjs — Comprehensive test suite for jobops
 *
 ...
 * any freshly cloned jobops checkout with only Node.js available.
 *
 ...
 *
 * @param {string} path - Path relative to the jobops repository root.
...
const scripts = [
  ...
  // tests inside their active jobops workspace.
...
if (!QUICK) {
  ...
  if (!hasGo) {
    ...
    if (isWindows && !goEnv.GOCACHE) {
      goEnv.GOCACHE = join(tmpdir(), 'jobops-go-build-cache');
    }
    if (goEnv.GOCACHE) {
...
const systemFiles = [
  ...
  '.claude/skills/jobops/SKILL.md',
  '.opencode/skills/jobops/SKILL.md',
  '.qwen/skills/jobops/SKILL.md',
  '.antigravitycli/skills/jobops/SKILL.md',
  '.grok/skills/jobops/SKILL.md',
];
...
for (const skillPath of ['.claude/skills/jobops/SKILL.md', '.agents/skills/jobops/SKILL.md']) {
  if (!fileExists(skillPath)) {
    fail(`${skillPath} is missing`);
    continue;
  }
  const skill = readFile(skillPath);
  if (skill.includes('/jobops latex')) {
    pass(`${skillPath} exposes /jobops latex in discovery menu`);
  } else {
    fail(`${skillPath} does not expose /jobops latex in discovery menu`);
  }
}
...
try {
  ...
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'jobops-missing-pipeline-'));
...
try {
  const tmp = mkdtempSync(join(tmpdir(), 'jobops-portals-validator-'));
...
const canonicalSkill = '.agents/skills/jobops/SKILL.md';
const symlinks = [
  '.claude/skills/jobops/SKILL.md',
  '.opencode/skills/jobops/SKILL.md',
  '.qwen/skills/jobops/SKILL.md',
...
if (
  ...
  /\/jobops/.test(canonicalContent ?? '')
) {
  pass('jobops skill router documents the Codex invocation model');
} else {
  fail('jobops skill router is missing Codex invocation guidance');
}
...
if (
  ...
  /(slash commands?.*not guaranteed|prompt|\/jobops.*unavailable)/i.test(agentsDoc)
) {
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'jobops-skills-'));
  try {
    const canonicalDir = join(fixtureRoot, '.agents', 'skills', 'jobops');
    const claudeDir = join(fixtureRoot, '.claude', 'skills', 'jobops');
    const opencodeDir = join(fixtureRoot, '.opencode', 'skills', 'jobops');
    mkdirSync(canonicalDir, { recursive: true });
    mkdirSync(claudeDir, { recursive: true });
    mkdirSync(opencodeDir, { recursive: true });

    const fixtureSkill = '---\nname: jobops\n---\n\n# jobops skill\n';
    const pointer = '../../../.agents/skills/jobops/SKILL.md';
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'jobops-ensure-skills-'));
  try {
    const canonicalDir = join(fixtureRoot, '.agents', 'skills', 'jobops');
    const claudeDir = join(fixtureRoot, '.claude', 'skills', 'jobops');
    mkdirSync(canonicalDir, { recursive: true });
    ...
    const grokSkill = readFileSync(join(fixtureRoot, '.grok', 'skills', 'jobops', 'SKILL.md'), 'utf-8');
    const claudeSkill = readFileSync(join(claudeDir, 'SKILL.md'), 'utf-8');
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'jobops-skills-unreadable-'));
  try {
    const canonicalDir = join(fixtureRoot, '.agents', 'skills', 'jobops');
    const claudeDir = join(fixtureRoot, '.claude', 'skills', 'jobops');
    mkdirSync(canonicalDir, { recursive: true });
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'jobops-skills-entry-dir-'));
  try {
    const canonicalDir = join(fixtureRoot, '.agents', 'skills', 'jobops');
    const claudeDir = join(fixtureRoot, '.claude', 'skills', 'jobops');
    const opencodeDir = join(fixtureRoot, '.opencode', 'skills', 'jobops');
    ...
    const materialized = updater.materializeSkillEntrypoints(fixtureRoot);
    const opencodeSkill = readFileSync(join(opencodeDir, 'SKILL.md'), 'utf-8');
    if (JSON.stringify(materialized) === JSON.stringify(['.opencode/skills/jobops/SKILL.md']) && opencodeSkill === fixtureSkill) {
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'jobops-skill-git-'));
  ...
  try {
    ...
    writeFileSync(join(claudeDir, 'SKILL.md'), pointer);
    writeFileSync(join(opencodeDir, 'SKILL.md'), pointer);
    gitRun(['add', '--', '.agents/skills/jobops/SKILL.md']);

    const pointerBlob = gitRun(['hash-object', '-w', '--stdin'], { input: pointer });
    gitRun(['update-index', '--add', '--cacheinfo', `120000,${pointerBlob},.claude/skills/jobops/SKILL.md`]);
    gitRun(['update-index', '--add', '--cacheinfo', `120000,${pointerBlob},.opencode/skills/jobops/SKILL.md`]);
    ...
    const claudeIndex = gitRun(['ls-files', '-s', '--', '.claude/skills/jobops/SKILL.md']);
    const opencodeIndex = gitRun(['ls-files', '-s', '--', '.opencode/skills/jobops/SKILL.md']);
    ...
    const claudeBlob = gitRaw(['show', ':.claude/skills/jobops/SKILL.md']);
    const opencodeBlob = gitRaw(['show', ':.opencode/skills/jobops/SKILL.md']);
...
try {
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), 'jobops-migrate-'));
  try {
    ...
    run(NODE, ['merge-tracker.mjs', '--migrate'], { env: { ...process.env, JOBOPS_TRACKER: tracker } });
  ...
  const followupTmp = mkdtempSync(join(tmpdir(), 'jobops-followup-link-'));
  try {
...
try {
  const vpTmp = mkdtempSync(join(tmpdir(), 'jobops-verify-reports-'));
  try {
    const vpReports = join(vpTmp, 'reports');
    mkdirSync(vpReports, { recursive: true });
    const vpTracker = join(vpTmp, 'applications.md');
    const vpEnv = { ...process.env, JOBOPS_TRACKER: vpTracker, JOBOPS_REPORTS: vpReports };
...
try {
  ...
  const dedupTmp = mkdtempSync(join(tmpdir(), 'jobops-dedup-'));
  try {
    ...
    const dedupResult = run(NODE, ['dedup-tracker.mjs'], { env: { ...process.env, JOBOPS_TRACKER: tracker } });
    if (dedupResult === null) {
...
try {
  const rebuildTmp = mkdtempSync(join(tmpdir(), 'jobops-rebuild-'));
  try {
    ...
    const r = run(NODE, ['dedup-tracker.mjs'], { env: { ...process.env, JOBOPS_TRACKER: tracker } });
    if (r === null) {
...
try {
  const locTmp = mkdtempSync(join(tmpdir(), 'jobops-dedup-loc-'));
  try {
...
try {
  const mergeTmp = mkdtempSync(join(tmpdir(), 'jobops-merge-'));
  try {
    ...
    const mergeResult = run(NODE, ['merge-tracker.mjs'], { env: { ...process.env, JOBOPS_TRACKER: tracker, JOBOPS_ADDITIONS: additionsDir } });
    if (mergeResult === null) {
...
try {
  const col912Tmp = mkdtempSync(join(tmpdir(), 'jobops-merge-912-'));
  try {
    ...
    const col912Result = run(NODE, ['merge-tracker.mjs'], {
      env: { ...process.env, JOBOPS_TRACKER: col912Tracker, JOBOPS_ADDITIONS: col912Additions },
    });
...
try {
  const mergeTmp = mkdtempSync(join(tmpdir(), 'jobops-merge-lock-'));
  /**
  ...
  function spawnMerge(additionsDir, holdMs = 0) {
    ...
    const result = new Promise(resolve => {
      const child = spawn(NODE, ['merge-tracker.mjs'], {
        ...
        env: {
          ...process.env,
          JOBOPS_TRACKER: join(mergeTmp, 'data', 'applications.md'),
          JOBOPS_ADDITIONS: additionsDir,
          JOBOPS_TRACKER_LOCK: join(mergeTmp, 'jobops-merge-tracker-fixture.lock'),
          JOBOPS_MERGE_HOLD_MS: String(holdMs),
          JOBOPS_MERGE_READY_IPC: '1',
        },
...
if (!sqliteAvailable) {
  ...
  try {
    const idxTmp = mkdtempSync(join(tmpdir(), 'jobops-index-'));
    try {
      const md = join(idxTmp, 'applications.md');
      const env = { ...process.env, JOBOPS_TRACKER: md };
...
try {
  ...
  // Release Please tags carry the component prefix (jobops-v1.9.0); the
  // prefix must be stripped or the releases-API fallback is dead code (#923).
  if (parse('jobops-v1.9.0') === '1.9.0') {
    pass('SEMVER_RE parses Release Please component-prefixed tag (jobops-v1.9.0 → 1.9.0)');
  } else {
    fail(`SEMVER_RE failed on jobops-v1.9.0 (got ${parse('jobops-v1.9.0')}) — releases-API fallback is dead code (#923)`);
  }
  ...
  if (parse('jobops') === null && parse('v1.9') === null) {
    pass('SEMVER_RE rejects non-semver input');
  } else {
    fail(`SEMVER_RE matched non-semver input (jobops → ${parse('jobops')}, v1.9 → ${parse('v1.9')})`);
  }
} catch (e) {
...
try {
  ...
  const goodEntry = { name: 'jobops-plugin-x', id: 'x', repo: 'https://github.com/a/jobops-plugin-x', author: 'a', hooks: ['ingest'], requiredEnv: [], allowedHosts: ['api.x.com'], license: 'MIT', version: '1.0.0', sha: 'a'.repeat(40) };
  ...
  if (reg.validateRegistryEntry({ ...goodEntry, name: 'evil-x' }, regOpts).length > 0) pass('registry: name must start with jobops-plugin-');
  ...
  writeFileSync(join(succTmp, 'plugins-registry.json'), JSON.stringify({ registryVersion: 1, plugins: [{ name: 'jobops-plugin-gmail', id: 'gmail', repo: 'https://github.com/a/jobops-plugin-gmail', author: 'a', hooks: ['ingest'], requiredEnv: [], allowedHosts: [], license: 'MIT', version: '2.0.0', sha: SUCC_SHA, supersedesBundled: true }] }));
  ...
  if (reg.successorFor(succTmp, 'gmail')?.name === 'jobops-plugin-gmail') pass('successor: successorFor() surfaces the maintained version of a bundled id');
  else fail('successor: successorFor should return the registered successor');
  rmSync(succTmp, { recursive: true, force: true });

  if (install.parseRepoArg('alice/jobops-plugin-foo').id === 'foo') pass('install: owner/jobops-plugin-foo parses to id "foo"');
  ...
  if (nameRej) pass('install: refuses a repo not named jobops-plugin-*');
