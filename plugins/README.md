...
> Claude Code marketplace metadata). These plugins extend jobops itself.
...
A plugin is a directory under `plugins/` (bundled, shipped with jobops) or
...
jobops is plain ESM with no build step, so the engine **cannot truly
...
- **Centralized infrastructure** the project would operate — hosted job
  ...
  not the open-core.
- **Auto-submitting / blind-applying** to jobs. jobops is a decision-support
